...
public:
    ...
    Estudiante(MySQLConexion& conexion)
        : conn(conexion), orm(conexion, "ESTUDIANTES", {"NOMBRE", "APELLIDO", "CORREO", "FECHA_NACIMIENTO"}) {}
    ...
    void actualizar_estudiante() {
        ...
        alumno.set("id", to_string(id));
        alumno.set("NOMBRE", nombre);
        alumno.set("APELLIDO", apellido);
        alumno.set("CORREO", correo);
        alumno.set("FECHA_NACIMIENTO", fecha);

        cout << (alumno.update() ? "Estudiante actualizado.\n" : "Error al actualizar estudiante.\n");
    }
    /*
...
public:
    Docente(MySQLConexion& conexion)
        : conn(conexion), orm(conexion, "DOCENTES", {"NOMBRE", "ESPECIALIDAD"}) {}
    ...
    void actualizar_docente() {
     ...
     profesor.set("id", to_string(id));
     profesor.set("NOMBRE", nombre);
     profesor.set("APELLIDO", apellido);
     profesor.set("ESPECIALIDAD", especialidad);

        cout <<  profesor.update() ? "Docente actualizado.\n" : "Error al actualizar docente.\n";
    }
    /*
...
public:
    ...
    Curso(MySQLConexion& conexion)
        : conn(conexion), orm(conexion, "CURSOS", {"NOMBRE", "CREDITOS", "ID_DOCENTE"}) {}
    /*
    ...
    void actualizar_curso() {
        ...
        materia.set("id", to_string(id));
        materia.set("NOMBRE_CURSO", nombre);
        materia.set("DESCRIPCION", descripcion);
        materia.set("CREDITOS", to_string(creditos));
        materia.set("ID_DOCENTE", to_string(idDocente));

        cout << (materia.update() ? "Curso actualizado.\n" : "Error al actualizar curso.\n");
    }
    /*
...
public:
    Nota(MySQLConexion& conexion)
        : conn(conexion), orm(conexion, "NOTAS", {"ID_ESTUDIANTE", "ID_CURSO", "CALIFICACION"}) {}
    /*
    ...
    void crear_nota() {
        ...
        EloquentORM nota(conn, "NOTAS", {"ID_ESTUDIANTE", "ID_CURSO", "TIPO", "NOTA", "FECHA_REGISTRO"});
        // Solicitar datos al usuario
        ...
        nota.set("ID_ESTUDIANTE", to_string(idEstudiante));
        nota.set("ID_CURSO", to_string(idCurso));
        nota.set("NOTA", to_string(calificacion));
        nota.set("TIPO", tipo);
        nota.set("FECHA_REGISTRO", fecha);

        cout << (nota.create() ? "Nota registrada.\n" : "Error al registrar nota.\n");
    }
    /*
    ...
    void actualizar_nota() {
        EloquentORM nota(conn, "NOTAS", {"id", "ID_ESTUDIANTE", "ID_CURSO", "TIPO", "NOTA", "FECHA_REGISTRO"}); 
        ...
        cout << (nota.update() ? "Nota actualizada.\n" : "Error al actualizar nota.\n");
    }
    /*
    ...
    void eliminar_nota() {
        ...
        else if (nota.find(id)) {
        cout << "Se elimino la nota del estudiante con ID: " << nota.get("ID_ESTUDIANTE") << endl;
        nota.remove(); 
        }
        else {
            cout << "No se encontro un estudiante con el ID: " << id << endl;
